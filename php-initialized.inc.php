...
function check_variables($filename, $initialized = array(), $function = "", $class = "", $in_string = false, $tokens = null, $i = 0, $single_command = null) {
	...
	for (; $i < count($tokens); $i++) {
		...
		// blocks
			...
			if (isset($function_calls[count($function_calls) - 1][0]) && $function_calls[count($function_calls) - 1][0] !== '$...') {
		...
		if (count($function_calls) === $single_command && ($token === '{' || $token === ';') && isset($tokens[$i+1][0]) && !in_array($tokens[$i+1][0], array(T_ELSE, T_ELSEIF, T_CATCH), true)) {
			return $i;
		}
	}
	return $initialized;
